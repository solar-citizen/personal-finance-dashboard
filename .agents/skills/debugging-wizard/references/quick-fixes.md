...
// Fix 2: Proxy in development (Next.js)
// next.config.js
module.exports = {
  async rewrites() {
    return [
      {
        source: '/api/:path*',
        destination: 'http://localhost:3001/:path*', // your NestJS API
      },
    ];
  },
};
// Note: this only proxies in dev/when using rewrites — for a real deployment,
// the API's CORS config (or a reverse proxy) still needs to allow the origin.
```
