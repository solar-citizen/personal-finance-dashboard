...
- Configuring and using PostgreSQL extensions — notably `pgvector`, which is
  a first-class part of this project's stack (embeddings/AI chat features),
...
Note: the database is managed through Prisma (engineless). Schema changes
should go through Prisma migrations rather than hand-written DDL where a
Prisma-managed equivalent exists — but raw SQL is still the right tool for
EXPLAIN analysis, indexing decisions, extension setup, and anything Prisma's
schema DSL doesn't express directly (e.g. `vector` columns, HNSW indexes,
...
| Extensions | `references/extensions.md` | pgvector (core to this stack), pg_stat_statements |
